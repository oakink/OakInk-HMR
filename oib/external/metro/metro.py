...
from oib.external.metro.hrnet.config import config as hrnet_config
from oib.external.metro.hrnet.config import \
    update_config as hrnet_update_config
from oib.external.metro.hrnet.hrnet import get_cls_net
from oib.metrics.basic_metric import LossMetric
from oib.metrics.freihand_eval import PAEval
from oib.metrics.mean_epe import MeanEPE
from oib.models.layers import create_backbone
from oib.models.layers.mano_wrapper import kpId2vertices
...
from oib.viztools.draw import draw_batch_joint_images, plot_hand
...
class METRO(nn.Module, ModuleAbstract):
    ...
    def __init__(self, cfg):
        ...
        for i in range(len(output_feat_dim)):
            ...
            config = config_class.from_pretrained("neo/external/metro/bert_cfg.json")
        ...
        if cfg.BACKBONE == "hrnet":
            hrnet_yaml = "neo/external/metro/hrnet/config/cls_hrnet_w40_sgd_lr5e-2_wd1e-4_bs32_x100.yaml"
            ...
            hrnet_yaml = "neo/external/metro/hrnet/config/cls_hrnet_w64_sgd_lr5e-2_wd1e-4_bs32_x100.yaml"
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
        ...
        if "mano_layer.th_weights" not in state_dict:
            # load from METRO raw version
            new_dict = {}
            for k, v in state_dict.items():
                if "upsampling." not in k and "cam_param_fc" not in k:
                    new_dict[k] = v
                new_dict["metro_network." + str(k)] = v
            mano_layer = ManoLayer(joint_rot_mode="axisang",
                                   use_pca=False,
                                   mano_assets_root="assets/mano_v1_2",
                                   center_idx=0,
                                   flat_hand_mean=True)
            for k in [
                    "th_betas", "th_shapedirs", "th_posedirs", "th_v_template", "th_J_regressor", "th_weights",
                    "th_faces", "th_hands_mean"
            ]:
                new_dict["mano_layer." + k] = getattr(mano_layer, k)
            for i in range(3):
                new_dict[f"trans_encoder.{i}.bert.embeddings.position_ids"] = torch.arange(512)[None]
                new_dict[f"metro_network.trans_encoder.{i}.bert.embeddings.position_ids"] = torch.arange(512)[None]

            pop_key = list(state_dict.keys())
            for k in pop_key:
                state_dict.pop(k)
            state_dict.update(new_dict)
        super()._load_from_state_dict(new_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys,
                                      error_msgs)
