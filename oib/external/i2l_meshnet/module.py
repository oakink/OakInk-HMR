...
class PoseNet(nn.Module):
    def __init__(self, joint_num, output_hm_shape):
        ...
        self.conv_z_1 = make_conv1d_layers([2048, 256 * self.output_hm_shape[0]], kernel=1, stride=1, padding=0)
    ...
    def forward(self, img_feat):
        ...
        img_feat_z = img_feat_z.view(-1, 256, self.output_hm_shape[0])
...
class Pose2Feat(nn.Module):
    def __init__(self, joint_num, output_hm_shape):
        ...
        self.joint_num = joint_num
        self.output_hm_shape = output_hm_shape
        self.conv = make_conv_layers([64 + joint_num * self.output_hm_shape[0], 64])

    def forward(self, img_feat, joint_heatmap_3d):
        joint_heatmap_3d = joint_heatmap_3d.view(
            -1, self.joint_num * self.output_hm_shape[0], self.output_hm_shape[1], self.output_hm_shape[2]
        )
...
class MeshNet(nn.Module):
    def __init__(self, vertex_num, output_hm_shape):
...
class ParamRegressor(nn.Module):
    ...
    def forward(self, pose_3d):
        ...
        pose = torch.cat([pose, torch.zeros((pose.shape[0], 3, 1)).to(pose_3d.device).float()], 2)
        pose = tgm.rotation_matrix_to_angle_axis(pose).reshape(batch_size, -1)  # TODO alter this
