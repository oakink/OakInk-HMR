...
def spiral_tramsform(
    ...
    if not os.path.exists(transform_fp):
        ...
        _, A, D, U, F, V = generate_transform_matrices(mesh, ds_factors)
        tmp = {'vertices': V, 'face': F, 'adj': A, 'down_transform': D, 'up_transform': U}

        with open(transform_fp, 'wb') as fp:
            pickle.dump(tmp, fp)
        logger.info('Done!')
        logger.info(f'Transform matrices are saved in \'{transform_fp}\'')
    else:
        with open(transform_fp, 'rb') as f:
            tmp = pickle.load(f, encoding='latin1')

    spiral_indices_list = [
        preprocess_spiral(tmp['face'][idx], seq_length[idx], tmp['vertices'][idx], dilation[idx])  #.to(device)
...
def extract_spirals(mesh, seq_length, dilation=1):
    ...
    for vh0 in mesh.vertices():
        ...
        if len(next_ring) > 0:
            ...
            spiral = kdt.query(np.expand_dims(mesh.points()[spiral[0]], axis=0),
...
def to_sparse(spmat):
    return torch.sparse.FloatTensor(torch.LongTensor([spmat.tocoo().row, spmat.tocoo().col]),
