...
class ManoBranch(nn.Module):
    ...
    def forward(
        ...
        is_rights, is_lefts = is_rights.bool(), is_lefts.bool() # Remove the UserWarning
        ...
        if pose_right.shape[0] > 0:
            mano_right = self.mano_layer_right(
                pose_right,
                th_betas=shape_right,
                th_trans=trans_right,
                root_palm=root_palm,
            )
            verts_right, joints_right = mano_right.verts, mano_right.joints
        if pose_left.shape[0] > 0:
            mano_left = self.mano_layer_left(
                pose_left,
                th_betas=shape_left,
                th_trans=trans_left,
                root_palm=root_palm,
            )
            verts_left, joints_left = mano_left.verts, mano_left.joints
        if self.adapt_skeleton:
...
class ManoLoss:
    def __init__(
        self,
        lambda_verts=1e-3,
        lambda_joints3d=1e-3,
        lambda_shape=None,
        lambda_pose_reg=1e-3,
        lambda_pca=None,
        center_idx=9,
    ...
    def compute_loss(self, preds, target):
        ...
        if "target_verts_3d" in target and self.lambda_verts:
            verts3d_loss = torch_f.mse_loss(
                preds["verts"], target["target_verts_3d"]
            )
        ...
        if "target_joints_3d" in target:
            pred_joints = preds["joints"]
            target_joints = target["target_joints_3d"]
            ...
            if "target_joints_3d" in target and self.lambda_joints3d:
        ...
        if ("hand_pcas" in target) and self.lambda_pca:
            pca_loss = torch_f.mse_loss(
                preds["pcas"], target["hand_pcas"]
            )
