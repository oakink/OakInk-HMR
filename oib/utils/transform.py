...
class SimpleTransform2D:
    def __init__(
        ...
        super().__init__()
        self._scale_jit_factor = scale_jit_factor
        self._color_jit_factor = color_jit_factor
        self._rot_jit_factor = rot_jit_factor
        self._rot_prob = rot_prob
        ...
        if self._occlusion:
            self.occlusion_op = RandomOcclusion(occlusion_prob)
    ...
    def __call__(self, image, label):
        if self._aug:
            ...
            if self._occlusion:  # apply occlu
                occlu_inp = {}
                occlu_inp["bbox"] = center_scale_to_box(label["bbox_center"], bbox_scale)
        ...
        if not self._train:
            ...
            target_joints_vis = (
                ((tj2d[:, 0] >= 0) & (tj2d[:, 0] < self._output_size[0]))
                & ((tj2d[:, 1] >= 0) & (tj2d[:, 1] < self._output_size[1]))
            ).astype(np.float32)
            if target_joints_vis.sum() < CONST.NUM_JOINTS * 0.3:  # emprical number
                target_joints_vis = np.full(CONST.NUM_JOINTS, 0.0, dtype=np.float32)

        # deal with verts_2d
        has_verts_2d = "verts_2d" in label
        if has_verts_2d:
            target_verts_2d = _transform_coords(label["verts_2d"], affine).astype(np.float32)
            if not self._train:
                target_verts_vis = np.full(CONST.NUM_VERTS, 1.0, dtype=np.float32)
            else:
                tv2d = target_verts_2d
                target_verts_vis = (
                    ((tv2d[:, 0] >= 0) & (tv2d[:, 0] < self._output_size[0]))
                    & ((tv2d[:, 1] >= 0) & (tv2d[:, 1] < self._output_size[1]))
                ).astype(np.float32)
                if target_verts_vis.sum() < CONST.NUM_VERTS * 0.3:  # emprical number
                    target_verts_vis = np.full(CONST.NUM_VERTS, 0.0, dtype=np.float32)
        ...
        if has_verts_2d:
            results.update(
                {
                    "target_verts_2d": target_verts_2d,
                    "target_verts_vis": target_verts_vis,
                }
            )
...
class ObjectSimpleTransform2D:
    ...
    def __call__(self, image, label):
        ...
        target_corners_2d = _transform_coords(label["corners_2d"], affine).astype(np.float32)
        cv = label["corners_vis"]
        if not self._train:
            target_corners_vis = np.full(CONST.NUM_CORNERS, 1.0, dtype=np.float32)
        elif cv.sum() < CONST.NUM_CORNERS * 0.3:
            target_corners_vis = np.full(CONST.NUM_CORNERS, 0.0, dtype=np.float32)
        else:
            tc2d = target_corners_2d
            target_corners_vis = (
                ((tc2d[:, 0] >= 0) & (tc2d[:, 0] < self._output_size[0]))
                & ((tc2d[:, 1] >= 0) & (tc2d[:, 1] < self._output_size[1]))
            ).astype(np.float32)
            if target_corners_vis.sum() < CONST.NUM_JOINTS * 0.3:  # emprical number
                target_corners_vis = np.full(CONST.NUM_CORNERS, 0.0, dtype=np.float32)

        results = label
        results["target_corners_2d"] = target_corners_2d
        results["target_corners_vis"] = target_corners_vis

        return results


class SimpleTransform2DHeatmap(SimpleTransform2D):
    def __init__(self, heatmap_size, heatmap_sigma, **kwargs) -> None:
    ...
    def __call__(self, image, label):
        ...
        j2d = results["target_joints_2d"]  # (21, 2)
        nJ = j2d.shape[0]  # number of joints
        """ Generate GT Gussian heatmap for joints_2d"""
        target_j_heatmap = np.zeros((nJ, self._heatmap_size[1], self._heatmap_size[0]), dtype="float32")  # (nJ, H, W)
...
class SimpleTransformUVD(SimpleTransform2D):
    def __init__(self, center_idx, **kwargs) -> None:
        super().__init__(**kwargs)
...
class ObjectSimpleTransformUVD(ObjectSimpleTransform2D):
    def __init__(self, center_idx, **kwargs) -> None:
        super().__init__(**kwargs)
        self._center_idx = center_idx
    ...
    def __call__(self, image, label):
        ...
        verts_uv = label["obj_verts_uvd"][:, :2]
        corners_uv = label["corners_uvd"][:, :2]
        obj_verts_uv = _transform_coords(verts_uv, affine).astype(np.float32)
        corners_uv = _transform_coords(corners_uv, affine).astype(np.float32)

        root_joint_d = results["target_root_d"]
        verts_d = label["obj_verts_uvd"][:, 2:]
        corners_d = label["corners_uvd"][:, 2:]

        obj_verts_d_rel = verts_d - root_joint_d
        corners_d_rel = corners_d - root_joint_d

        tv_uv = obj_verts_uv / np.array([self._output_size[0], self._output_size[1]])
        tv_d = 0.5 + (obj_verts_d_rel[:, :1] / CONST.UVD_DEPTH_RANGE)
        target_obj_verts_uvd = np.concatenate([tv_uv, tv_d], axis=1).astype(np.float32)

        tc_uv = corners_uv / np.array([self._output_size[0], self._output_size[1]])
        tc_d = 0.5 + (corners_d_rel[:, :1] / CONST.UVD_DEPTH_RANGE)
        target_corners_uvd = np.concatenate([tc_uv, tc_d], axis=1).astype(np.float32)

        results["target_obj_verts_uvd"] = target_obj_verts_uvd
        results["target_corners_uvd"] = target_corners_uvd
        return results


class SimpleTransformUVDOrthoMANO(SimpleTransformUVD):
    def __call__(self, image, label):
        # TODO
        results = super().__call__(image, label)
        rot = results["rot_rad"]
        ...
        # fit a new orthogonal intrinsic. shape: (3,)
        jt2d = results["target_joints_2d"]
        target_ortho_intr = fit_ortho_param(target_joints_3d_rel, jt2d)

        results["target_ortho_intr"] = target_ortho_intr
...
class SimpleTransform3D(SimpleTransformUVD):
    ...
    def __call__(self, image, label):
        ...
        cc = label["cam_center"]
        affine_postrot = _affine_transform_post_rot(
            center=center, scale=scale, optical_center=cc, out_res=self._output_size, rot=rot
        )
...
class SimpleTransform3DMANO(SimpleTransformUVD):
    """Apply 2D and 3D transform to images and labels, including:
    1. scale and color jittering;
    2. random ratation on image, 2d/3d keypoints, MANO global rotation;
    3. random occlusion on image;
...
class ObjectSimpleTransform3DMANO(ObjectSimpleTransformUVD):
    def __init__(self, **kwargs) -> None:
        super().__init__(**kwargs)
    ...
    def __call__(self, image, label):
        ...
        results["target_obj_verts_3d"] = rot_mat.dot(label["obj_verts_3d"].transpose(1, 0)).transpose()
        results["target_corners_3d"] = rot_mat.dot(label["corners_3d"].transpose(1, 0)).transpose()

        rot_mat4 = np.eye(4, dtype=np.float32)
        rot_mat4[:3, :3] = rot_mat
        results["target_obj_transf"] = rot_mat4 @ label["obj_transf"]

        # TODO
        try:
            obj_sdf = label["obj_sdf"]
            obj_sdf_points = rot_mat.dot(obj_sdf[:, :3].transpose(1, 0)).transpose()

            obj_sdf_points_rel = obj_sdf_points - rot_mat.dot(label["obj_transf"][:3, 3])

            results["target_obj_sdf"] = np.concatenate([obj_sdf_points, obj_sdf[:, 3:]], axis=1)
            results["target_obj_sdf_rel"] = np.concatenate([obj_sdf_points_rel, obj_sdf[:, 3:]], axis=1)
        except:
            pass

        results["target_obj_verts_3d_rel"] = results["target_obj_verts_3d"] - results["target_root_joint"]
        results["target_corners_3d_rel"] = results["target_corners_3d"] - results["target_root_joint"]
