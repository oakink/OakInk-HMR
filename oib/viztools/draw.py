...
import numpy as np
from oib.utils.heatmap import sample_with_heatmap
from oib.utils.transform import bchw_2_bhwc, denormalize

from .misc import COLOR_CONST


def draw_batch_joint_images(joints2d, gt_jointd2d, tensor_image, n_sample=16):
...
def draw_batch_corner_images(corners2d, gt_corners2d, tensor_image, n_sample=16):
    batch_size = corners2d.shape[0]
    if n_sample >= batch_size:
        n_sample = batch_size
    ...
    corners2d = corners2d[:n_sample, ...].detach().cpu().numpy()  # (B, NJ, 2)
    gt_corners2d = gt_corners2d[:n_sample, ...].detach().cpu().numpy()  # (B, NJ, 2)

    sample_list = []
    for i in range(n_sample):
        corners_img = plot_corner(image[i].copy(), corners2d[i])
        gt_corners_img = plot_corner(image[i].copy(), gt_corners2d[i])
        sample = np.hstack([corners_img, gt_corners_img])
        sample = cv2.copyMakeBorder(sample, 5, 5, 5, 5, cv2.BORDER_CONSTANT, value=(255, 255, 255))
        sample_list.append(sample[None, ...])
...
def concat_imgs(img):
    ...
    if remainder != 0:
        img = np.concatenate(
            [img, np.ones((4 - remainder, *img.shape[1:]), dtype=np.uint8) * 255],
            axis=0,
        )
    for i in range(len(img) // 4):
        image.append(img[i * 4 : (i + 1) * 4].reshape(4 * img.shape[1], img.shape[2], img.shape[3]))
...
def plot_hand(image, coords_hw, vis=None, linewidth=3):
    ...
    return plot_kps(image, coords_hw, bones, vis, linewidth)


def plot_corner(image, coords_hw, vis=None, linewidth=3):
    """Plots a hand stick figure into a matplotlib figure."""
