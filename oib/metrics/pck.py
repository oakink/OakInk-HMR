...
class Hand3DPCK(_PCKMetric):
    num_kp = CONST.NUM_JOINTS

    def __init__(self, cfg) -> None:
        super().__init__(cfg)
        self.eval_type = cfg.get("EVAL_TYPE", "joints_3d_rel")
        self.reset()

    def _get_predictions(self, preds: Dict, targs: Dict):
        if self.eval_type == "joints_3d_rel":
            pred_jts_rel = preds["pred_joints_rel"]  # TODO
            gt_jts_rel = targs["target_joints_3d_rel"]
            return pred_jts_rel, gt_jts_rel, targs["target_joints_vis"]

        elif self.eval_type == "joints_3d":
            pred_jts = preds["joints_3d"]
            gt_jts = targs["target_joints_3d"]
            return pred_jts, gt_jts, targs["target_joints_vis"]
