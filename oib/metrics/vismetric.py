...
class Vis2DMetric(VisMetric):
    ...
    def __init__(self, **cfg) -> None:
        ...
        logger.info(f"Build Vis2DMetric with grid {self.ncol} x {self.nrow}")

    def feed(self, preds: Dict, targs: Dict, **kwargs):
        batch_size = targs["image_np"].shape[0]
        if self.count > 0:
            self.count += batch_size
            return

        # ==== prepare data >>>>
        image = targs["image_np"].cpu()
        pred_joints_3d_rel = preds["pred_joints_rel"].detach().cpu()
        pred_joints_3d = pred_joints_3d_rel + targs["target_root_joint"].unsqueeze(1).cpu()
        gt_cam_intr = targs["target_cam_intr"].cpu()
        pred_joints_2d = transform.batch_persp_project(pred_joints_3d, gt_cam_intr)
        gt_joints_2d = targs["target_joints_2d"][:, :CONST.NUM_JOINTS, :].cpu()
        gt_joints_vis = targs["target_joints_vis"].cpu()
        gt_root = gt_joints_2d[:, self.center_idx, :]

        # ==== draw pred and gt >>>>
        pred_image = self.draw_batch_joints_image(batch_images=image,
                                                  batch_joints2d=pred_joints_2d,
                                                  batch_gt_root2d=gt_root,
                                                  batch_joints_vis=gt_joints_vis)
        ref_image = self.draw_batch_joints_image(batch_images=image,
                                                 batch_joints2d=gt_joints_2d,
                                                 batch_gt_root2d=gt_root,
                                                 batch_joints_vis=gt_joints_vis)
