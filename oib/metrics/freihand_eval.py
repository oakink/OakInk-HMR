...
from ..utils.builder import METRIC
...
class PAEval(Metric):
    def __init__(self, cfg) -> None:
        ...
        self.mesh_score = cfg.MESH_SCORE
    ...
    def feed(self, preds, targs, **kwargs):
        pred_joints_3d_abs = preds["joints_3d"]
        joints_3d_abs = targs["target_joints_3d"]
        ...
        if self.mesh_score:
            pred_verts_3d_abs = preds["verts_3d"]
            verts_3d_abs = targs["target_verts_3d"]
    ...
    def get_measures(self, **kwargs) -> Dict[str, float]:
        ...
        if self.mesh_score:
            measures["pa_mpvpe"] = self.pa_mpvpe.avg * 1000.0
            measures["mpvpe"] = self.mpvpe.avg * 1000.0
        return measures

    def get_result(self):
        return self.pa_mpjpe.avg * 1000.0

    def __str__(self) -> str:
        score = f"pa_mpjpe: {self.pa_mpjpe.avg * 1000.0:6.4f} | mpjpe: {self.mpjpe.avg * 1000.0:6.4f}"
        if self.mesh_score:
            score += f" | pa_mpvpe: {self.pa_mpvpe.avg * 1000.0:6.4f} | mpvpe: {self.mpvpe.avg * 1000.0:6.4f}"
