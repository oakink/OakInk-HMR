...
class ResNet(nn.Module):
    def __init__(self, version, block, layers, num_classes=1000, interm_feat=True, freez_batchnorm=True):
    ...
    def _load_from_state_dict(
        ...
        if self.interm_feat:
            try:
                del state_dict[prefix + "fc.weight"]
                del state_dict[prefix + "fc.bias"]
            except KeyError:
                logger.warning("No fc layer in pretrained model")
                pass

        super(ResNet, self)._load_from_state_dict(
            state_dict, prefix, local_metadata, strict, missing_keys, unexpected_keys, error_msgs
        )
...
def build_resnet(cfg):
    ...
    return build_model[cfg.TYPE](
        pretrained=cfg.PRETRAINED,
        freez_batchnorm=cfg.FREEZE_BATCHNORM,
        interm_feat=cfg.INTERM_FEAT if hasattr(cfg, "INTERM_FEAT") else True,
    )
