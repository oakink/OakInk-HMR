...
class ManoLayer:
    ...
    def __init__(
        ...
        self.betas = npj.array(smpl_data["betas"])[npj.newaxis, ...]
        self.shapedirs = npj.array(smpl_data["shapedirs"])
        self.posedirs = npj.array(smpl_data["posedirs"])
        self.v_template = npj.array(smpl_data["v_template"])[npj.newaxis, ...]
        self.J_regressor = npj.array(smpl_data["J_regressor"].toarray())
        self.weights = npj.array(smpl_data["weights"])
        self.faces = npj.array(smpl_data["f"])

        # Get hand mean
        hands_mean = npj.zeros(hands_components.shape[1]) if flat_hand_mean else smpl_data["hands_mean"]
        self.hands_mean = hands_mean.copy()[npj.newaxis, ...]  # 45 all zeros
        selected_components = hands_components[:ncomps]
        self.tselected_comps = npj.array(selected_components)
    ...
    def _ready_arguments(self, fname_or_dict, posekey4vposed="pose"):
        ...
        if "trans" not in dd:
            dd["trans"] = npj.zeros(3)
        if "pose" not in dd:
            dd["pose"] = npj.zeros(nposeparms)
        if "shapedirs" in dd and "betas" not in dd:
            dd["betas"] = npj.zeros(dd["shapedirs"].shape[-1])
        ...
        if want_shapemodel:
            ...
            dd["J"] = npj.vstack((J_tmpx, J_tmpy, J_tmpz)).T
    ...
    def _lrotmin(self, p):
        ...
        return npj.concatenate(
            [(cv2.Rodrigues(np.asarray(pp))[0] - npj.eye(3)).ravel() for pp in p.reshape((-1, 3))]
        ).ravel()
    ...
    def _subtract_flat_id(self, rot_mats):
        ...
        id_flat = npj.tile(npj.eye(3, dtype=rot_mats.dtype).reshape(1, 9), (1, rot_nb))
    ...
    def _quat2mat(self, quat):
        norm_quat = quat
        norm_quat = norm_quat / npj.linalg.norm(norm_quat + 1e-8, axis=1, keepdims=True)
    ...
    def _batch_rodrigues(self, axisang):
        # axisang N x 3
        axisang_norm = npj.linalg.norm(axisang + 1e-8, axis=1)
        angle = axisang_norm[..., npj.newaxis]
        axisang_normalized = axisang / angle
        angle = angle * 0.5
        v_cos = npj.cos(angle)
        v_sin = npj.sin(angle)
        quat = npj.concatenate((v_cos, v_sin * axisang_normalized), 1)
    ...
    def _with_zeros(self, tensor):
        ...
        padding = npj.array([0.0, 0.0, 0.0, 1.0])

        concat_list = (tensor, npj.tile(padding.reshape(1, 1, 4), (batch_size, 1, 1)))
        cat_res = npj.concatenate(concat_list, 1)
        return cat_res
    ...
    def __call__(
        ...
        full_pose = npj.concatenate((pose_coeffs[:, : self.rot], self.hands_mean + full_hand_pose), 1)
        ...
        v_shaped = npj.matmul(self.shapedirs, betas.transpose((1, 0))).transpose((2, 0, 1)) + self.v_template
        j = npj.matmul(self.J_regressor, v_shaped)
        # th_pose_map should have shape 20x135
        v_posed = v_shaped + npj.matmul(self.posedirs, pose_map.transpose((1, 0))[npj.newaxis, ...]).transpose(
            (2, 0, 1)
        )
        ...
        root_trans = self._with_zeros(npj.concatenate((root_rot, root_j), 2))
        ...
        # From base to tips
        # Get lev1 results
        all_transforms = [root_trans[:, npj.newaxis, ...]]
        lev1_j_rel = lev1_j - root_j.transpose((0, 2, 1))
        lev1_rel_transform_flt = self._with_zeros(
            npj.concatenate((lev1_rots, lev1_j_rel[..., npj.newaxis]), 3).reshape(-1, 3, 4)
        )
        root_trans_flt = npj.tile(root_trans[:, npj.newaxis, ...], (1, 5, 1, 1)).reshape(root_trans.shape[0] * 5, 4, 4)
        lev1_flt = npj.matmul(root_trans_flt, lev1_rel_transform_flt)
        ...
        lev2_rel_transform_flt = self._with_zeros(
            npj.concatenate((lev2_rots, lev2_j_rel[..., npj.newaxis]), 3).reshape(-1, 3, 4)
        )
        lev2_flt = npj.matmul(lev1_flt, lev2_rel_transform_flt)
        ...
        lev3_rel_transform_flt = self._with_zeros(
            npj.concatenate((lev3_rots, lev3_j_rel[..., npj.newaxis]), 3).reshape(-1, 3, 4)
        )
        lev3_flt = npj.matmul(lev2_flt, lev3_rel_transform_flt)
        all_transforms.append(lev3_flt.reshape(all_rots.shape[0], 5, 4, 4))

        reorder_idxs = [0, 1, 6, 11, 2, 7, 12, 3, 8, 13, 4, 9, 14, 5, 10, 15]
        results = npj.concatenate(all_transforms, 1)[:, reorder_idxs]
        results_global = results

        joint_js = npj.concatenate((j, npj.zeros((j.shape[0], 16, 1))), 2)

        tmp2 = npj.matmul(results, joint_js[..., npj.newaxis])
        results2 = (results - npj.concatenate((npj.zeros((*tmp2.shape[:2], 4, 3)), tmp2), 3)).transpose((0, 2, 3, 1))

        T = npj.matmul(results2, self.weights.transpose((1, 0)))

        rest_shape_h = npj.concatenate((v_posed.transpose((0, 2, 1)), npj.ones((batch_size, 1, v_posed.shape[1]))), 1)

        verts = (T * rest_shape_h[:, npj.newaxis, ...]).sum(2).transpose((0, 2, 1))
        verts = verts[:, :, :3]
        jtr = results_global[:, :, :3, 3]
        # In addition to MANO reference joints we sample vertices on each finger
        # to serve as finger tips
        tips = verts[:, [745, 317, 444, 556, 673]]
        jtr = npj.concatenate((jtr, tips), 1)

        # Reorder joints to match visualization utilities
        jtr = jtr[:, [0, 13, 14, 15, 16, 1, 2, 3, 17, 4, 5, 6, 18, 10, 11, 12, 19, 7, 8, 9, 20]]

        center_joint = jtr[:, self.center_idx][:, npj.newaxis, ...]
